...
Reads the per-account Phase 2 categorized CSV (with terminal + journey labels),
runs the Phase 3 transform, and renders:

  * Sidebar filters (Payor Name, Primary DX, LLM Terminal Root Cause, Journey Length)
  ...
  * Horizontal bar of average touches by terminal root cause
  * Horizontal bar of average touches by journey length (the multi-note insight)
  * Audit table with full chronological note history per account
"""
