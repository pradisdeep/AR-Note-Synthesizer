...
class RedactionStats:
    ...
    def total(self) -> int:
        return self.insurance_names + self.member_ids + self.ssns + self.dobs
...
def redact_text(text: str, stats: RedactionStats) -> str:
...
def redact_dataframe(df: pd.DataFrame, note_col: str = "notescurrentvalue") -> tuple[pd.DataFrame, RedactionStats]:
    ...
    for original in out[note_col].tolist():
        new_value = redact_text(original, stats)
...
# Synthetic test fixtures only. These are NOT real names or IDs - they
# exercise the same regex patterns observed in production note data without
# embedding any real PHI in the source code. Format intentionally matches
# the real strings: ALL-CAPS Last,First inside CLAIMAUTOUPLOAD-style sentences
# and alphanumeric member-id substrings of typical length.
...
def selftest() -> None:
    ...
    print(stats.report())
    print()
    print("All samples (BEFORE -> AFTER):")
    for before, after in zip(df["notescurrentvalue"], redacted["notescurrentvalue"]):
